...
function teluro_page_builder_components( $components ) {
...
function teluro_default_components( $components ) {
...
function teluro_register_components( $components = array() ) {
    if ( apply_filters( 'colibri_page_builder/installed', false ) ) {
        $components = teluro_page_builder_components( $components );
    } else {
        $components = teluro_default_components($components);
    }

    return $components;
}

Hooks::prefixed_add_action('components', 'teluro_register_components');
Theme::load(array(
    'themeRelativePath' => '',
 ...
 */
function teluro_assets() {
    return teluro_theme()->getAssetsManager();
}
...
if (!apply_filters('colibri_page_builder/installed', false)) {
    teluro_assets()
        ...
        ->registerStylesheet("teluro-theme", "/theme/theme.css")
...
function teluro_show_post_meta_setting_filter( $value ) {
...
add_filter( 'colibri_show_post_meta', 'teluro_show_post_meta_setting_filter' );


function teluro_posts_per_row_setting_filter( $value ) {
...
add_filter( 'colibri_posts_per_row', 'teluro_posts_per_row_setting_filter' );

function teluro_archive_post_highlight_setting_filter( $value ) {
...
add_filter( 'colibri_archive_post_highlight', 'teluro_archive_post_highlight_setting_filter' );


function teluro_blog_sidebar_enabled_setting_filter( $value ) {
    $value = get_theme_mod( 'blog_sidebar_enabled', $value );

    return ( $value == 1 );
}

Hooks::prefixed_add_filter( 'blog_sidebar_enabled', 'teluro_blog_sidebar_enabled_setting_filter' );

function teluro_override_with_thumbnail_image( $value ) {
    global $post;
...
add_filter( 'colibri_override_with_thumbnail_image', 'teluro_override_with_thumbnail_image' );

function teluro_print_archive_entry_class( $class = "" ) {
    ...
    $postsPerRow = max( 1, apply_filters( 'teluro_posts_per_row', $default ) );


    $classes[] = "h-col-sm-12 h-col-md-" . ( 12 / intval( $postsPerRow ) );

    $classes = apply_filters( 'teluro_archive_entry_class', $classes );
...
function teluro_print_masonry_col_class( $echo = false ) {

    global $wp_query;
    $index        = $wp_query->current_post;
    $hasBigClass  = (is_sticky() || ($index === 0 && apply_filters('teluro_archive_post_highlight', false)));
...
Hooks::prefixed_add_filter( 'info_page_tabs', 'teluro_get_started_info_page_tab' );

function teluro_get_started_info_page_tab( $tabs ) {
...
function teluro_theme_plugins( $plugins ) {
    $theme_plugins = array();
...
Hooks::prefixed_add_filter( 'theme_plugins', 'teluro_theme_plugins' );


add_filter( 'http_request_host_is_external', 'teluro_allow_internal_host', 10, 3 );
function teluro_allow_internal_host( $allow, $host, $url ) {
...
add_action('wp_ajax_teluro_front_set_predesign', function () {
...
function teluro_override_main_row_class( $classes ) {
    return Defaults::get( 'templates.blog.row.layout-classes', $classes );
}

Hooks::prefixed_add_filter('main_row_class', 'teluro_override_main_row_class', 10, 1);
require_once __DIR__ . "/integration/colibri-page-builder/colibri-page-builder-integration.php";


add_filter('colibri_page_builder/remote_base_url', function ($url) {
    return 'http://colibriwp.com/defaults/?date=27-jan-21';
});

add_action('widgets_init', 'teluro_register_sidebars');

function teluro_register_sidebars()
{
    register_sidebar(
        array(
            'name'          => esc_html__('Blog sidebar widget area', 'teluro'),
    ...
    if (function_exists('\is_shop')) {
        register_sidebar(
            array(
                'name'          => esc_html__('WooCommerce left sidebar widget area', 'teluro'),
                'id'            => 'teluro-ecommerce-left',
                'before_widget' => '<div id="%1$s" class="widget %2$s">',
                'before_title'  => '<h5 class="widgettitle">',
                'after_title'   => '</h5>',
                'after_widget'  => '</div>',
